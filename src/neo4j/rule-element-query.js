...
MATCH (re:Rule_Element {rule_element_doc_id: $rule_element_doc_id})
  CALL {
    ...
    OPTIONAL MATCH (u:User {user_email: $userEmail })-[r1:USER_HAS_FAVORITE]->(res)
    WITH r1, id(resv) as version_of_id, res, sls, lang order by res.rule_element_in_force_from ASC, res.rule_element_in_force_until ASC
    RETURN collect({ isFavorite: id(r1), res: res, lang: lang, sls: sls, version_of_id: version_of_id }) as res
  }
RETURN re, res
`;
...
MATCH (rbi)<-[:HAS_RULE_BOOK_ISSUE]-(rb:Rule_Book)
...
CALL {
  ...
  OPTIONAL MATCH (rbi)-[:HAS_RULE_BOOK_ISSUE_STATE]->(rbis:Rule_Book_Issue_State)-[:RULE_BOOK_ISSUE_LANGUAGE_IS]->(lang)
  WITH collect({ identity: id(state), rule_element_doc_id: re.rule_element_doc_id , iso_639_1: lang.iso_639_1, rule_book_issue_title_short: rbis.rule_book_issue_title_short, rule_element_article: state.rule_element_article}) as res_new
  RETURN res_new
}
...
WHERE id(re) = $identity
  CALL {
    ...
    WITH re, CASE WHEN re_amc IS NULL THEN null ELSE collect({identity:id(re_amc), rule_element_doc_id: re_amc.rule_element_doc_id, order: r1.order, res: res }) END as amc
  ...
  CALL {
    ...
    WITH re, CASE WHEN re_gm IS NULL THEN null ELSE collect({identity:id(re_gm), rule_element_doc_id: re_gm.rule_element_doc_id, order: r2.order, res: res }) END as gm
