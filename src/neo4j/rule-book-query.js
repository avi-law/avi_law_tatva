...
exports.ruleBookStructQuery = (queryParams) => {
  let query = `
  ...
  if (!queryParams.isUpdate) {
    ...
    query = `
    ${query}
    MATCH (rbs:Rule_Book_Struct { rule_book_struct_id: "${queryParams.rule_book_struct_id}" })
    SET rbs.rule_book_struct_id = "${queryParams.rbs.rule_book_struct_id}"`;
  }
  ...
  if (queryParams.isUpdate) {
    ...
    CALL {
      WITH rbs
      OPTIONAL MATCH (rbs)-[r1:HAS_RULE_BOOK_STRUCT_CHILD]->()
      DETACH DELETE r1
      RETURN r1
    }
...
exports.ruleBookQuery = (queryParams) => {
  let query = ``;
  ...
  if (!queryParams.isUpdate) {
    ...
    query = `
    ${query}
    MATCH (rb:Rule_Book { rule_book_id: ${queryParams.rule_book_id} })
    SET rb.rule_book_id = ${queryParams.rb.rule_book_id}`;
  }
  ...
  if (queryParams.isUpdate) {
    ...
    CALL {
      WITH rb
      OPTIONAL MATCH (rb)-[r1:RULE_BOOK_BELONGS_TO_STRUCT]->()
      DETACH DELETE r1
      RETURN r1
    }
    ...
    RETURN rb`;
      ...
      CALL {
        WITH rb
        OPTIONAL MATCH (rb)-[r1:RULE_BOOK_CHILD]->()
        DETACH DELETE r1, r2
        RETURN r1
      }
  ...
  } else if (queryParams.rule_book_parent_struct_id) {
    query = `${query}
    MERGE (rb)-[:RULE_BOOK_BELONGS_TO_STRUCT{order_rule_book_in_struct: ${queryParams.rule_book_struct_order}}]->(rbs)
    RETURN rb`;
  } else if (queryParams.rule_book_parent_id) {
    query = `${query}
    MERGE (rb)-[:RULE_BOOK_CHILD {order_rule_book_child: ${queryParams.rule_book_child_order}}]->(rbp)
...
exports.ruleBookIssueQuery = (queryParams) => {
  let query = `
  ...
  if (queryParams.isUpdate) {
    ...
    CALL {
      WITH rbi
      OPTIONAL MATCH (rbi)-[r1:HAS_RULE_BOOK_ISSUE]->()
      DETACH DELETE r1
      RETURN r1
    }
