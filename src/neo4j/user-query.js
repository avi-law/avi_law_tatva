...
exports.getUserHistoryLogList = (lastLogId = null) => {
  let query = `MATCH (lt:Log_Type)<-[:HAS_LOG_TYPE]-(log:Log)-[:LOG_FOR_USER]->(u:User {user_email: $user_email})
  WHERE lt.log_type_id IN [${constants.LOG_TYPE_ID.READ_RULE_BOOK},${constants.LOG_TYPE_ID.READ_RULE_ELEMENT_AND_STATE},${constants.LOG_TYPE_ID.READ_NL}]`;
  if (lastLogId) {
    query = `
    ${query} AND id(log) < ${lastLogId}`;
  }
  query = `
  ${query}
  WITH lt, log ORDER BY log.log_timestamp DESC LIMIT toInteger($limit)
  MATCH (log)-[:LOG_REFERS_TO_OBJECT]->(obj)
  WITH lt, log, obj
  CALL apoc.do.case([
    lt.log_type_id = ${constants.LOG_TYPE_ID.READ_RULE_ELEMENT_AND_STATE} AND LABELS(obj) = ["${constants.LOG_REFERS_TO_OBJECT_LABEL.RULE_ELEMENT_STATE}"],
      'MATCH (lang:Language)<-[:RULE_ELEMENT_STATE_LANGUAGE_IS]-(obj)<-[:HAS_RULE_ELEMENT_STATE]-(re:Rule_Element)
      OPTIONAL MATCH (obj)-[r:RULE_ELEMENT_STATE_LANGUAGE_VERSION_OF]->(resv:Rule_Element_State)
      MATCH (re),(rbi:Rule_Book_Issue),
      path = shortestPath((re)<-[:HAS_RULE_ELEMENT*]-(rbi))
      WITH *, re, path, MIN(length(path)) as minLength ORDER BY minLength ASC LIMIT 1
      WITH *, nodes(path)[minLength] as rbi
      ...
      WITH collect({ label: LABELS(obj)[0], minLength: minLength, lt: properties(lt), log: { identity: id(log), log_par_01: log.log_par_01, log_timestamp: log.log_timestamp }, version_of_id: id(resv), res: { identity: id(obj), rule_element_title: obj.rule_element_title, rule_element_article: obj.rule_element_article, rule_element_doc_id: re.rule_element_doc_id, rule_book_issue_title_short: rbis.rule_book_issue_title_short }, iso_639_1: lang.iso_639_1}) as data
      RETURN data',
    lt.log_type_id = ${constants.LOG_TYPE_ID.READ_RULE_ELEMENT_AND_STATE} AND LABELS(obj) = ["${constants.LOG_REFERS_TO_OBJECT_LABEL.RULE_ELEMENT}"],
      'MATCH (lang:Language)<-[:RULE_ELEMENT_STATE_LANGUAGE_IS]-(res:Rule_Element_State)<-[:HAS_RULE_ELEMENT_STATE]-(obj)
      OPTIONAL MATCH (res)-[r:RULE_ELEMENT_STATE_LANGUAGE_VERSION_OF]->(resv:Rule_Element_State)
      MATCH (obj),(rbi:Rule_Book_Issue),
      path = shortestPath((obj)<-[:HAS_RULE_ELEMENT*]-(rbi))
      WITH *, obj, path, MIN(length(path)) as minLength ORDER BY minLength ASC LIMIT 1
      WITH *, nodes(path)[minLength] as rbi
      MATCH (rb:Rule_Book)-[:HAS_RULE_BOOK_ISSUE]->(rbi)-[:HAS_RULE_BOOK_ISSUE_STATE]->(rbis:Rule_Book_Issue_State)-[:RULE_BOOK_ISSUE_LANGUAGE_IS]->(lang)
      WITH collect({ label: LABELS(obj)[0], minLength: minLength, lt: properties(lt), log: { identity: id(log), log_par_01: log.log_par_01, log_timestamp: log.log_timestamp }, version_of_id: id(resv), res: { identity: id(res), rule_element_title: res.rule_element_title, rule_element_article: res.rule_element_article, rule_element_doc_id: obj.rule_element_doc_id, rule_book_issue_title_short: rbis.rule_book_issue_title_short }, iso_639_1: lang.iso_639_1}) as data
      RETURN data',
    lt.log_type_id = ${constants.LOG_TYPE_ID.READ_RULE_BOOK} AND LABELS(obj) = ["${constants.LOG_REFERS_TO_OBJECT_LABEL.RULE_BOOK}"],
      'MATCH (obj)-[:HAS_RULE_BOOK_ISSUE]->(rbi:Rule_Book_Issue)-[:HAS_RULE_BOOK_ISSUE_STATE]->(rbis:Rule_Book_Issue_State)-[:RULE_BOOK_ISSUE_LANGUAGE_IS]->(lang:Language)
      WITH collect({ label: LABELS(obj)[0], lt: properties(lt), log: { identity: id(log), log_par_01: log.log_par_01, log_timestamp: log.log_timestamp }, rb:{ identity: id(obj), rule_book_id: obj.rule_book_id, rule_book_issue_title_short: rbis.rule_book_issue_title_short, rule_book_issue_title_long: rbis.rule_book_issue_title_long }, iso_639_1: lang.iso_639_1 }) as data
      RETURN data',
    lt.log_type_id = ${constants.LOG_TYPE_ID.READ_NL} AND LABELS(obj) = ["${constants.LOG_REFERS_TO_OBJECT_LABEL.NL}"],
      'MATCH (cou:Country)<-[:NL_REFERS_TO_COUNTRY]-(obj)-[:HAS_NL_STATE]->(nls:Nl_State)-[:NL_LANG_IS]->(lang:Language)
      WITH collect({ label: LABELS(obj)[0], lt: properties(lt), log: { identity: id(log), log_par_01: log.log_par_01, log_timestamp: log.log_timestamp }, nl: { nl: properties(obj), nls: { nl_title_short: nls.nl_title_short, nl_title_long: nls.nl_title_long } }, iso_639_1: lang.iso_639_1 }) as data
      RETURN data'
  ],
  "",
  { obj: obj, lt: lt, log: log })
  YIELD value
  WITH value.data as logs
  RETURN logs`;
  return query;
};
...
FOREACH (_ IN CASE WHEN next_order IS NULL AND res IS NOT NULL AND u IS NOT NULL THEN [1] END | MERGE (u)-[:USER_HAS_FAVORITE { order: 1, favorite_timestamp: apoc.date.currentTimestamp() }]->(res))
FOREACH (_ IN CASE WHEN next_order IS NOT NULL AND res IS NOT NULL AND u IS NOT NULL THEN [1] END | MERGE (u)-[:USER_HAS_FAVORITE { order: next_order, favorite_timestamp: apoc.date.currentTimestamp() }]->(res))
RETURN res, u
`;
...
FOREACH (_ IN CASE WHEN next_order IS NULL AND rb IS NOT NULL AND u IS NOT NULL THEN [1] END | MERGE (u)-[:USER_HAS_FAVORITE { order: 1, favorite_timestamp: apoc.date.currentTimestamp() }]->(rb))
FOREACH (_ IN CASE WHEN next_order IS NOT NULL AND rb IS NOT NULL AND u IS NOT NULL THEN [1] END | MERGE (u)-[:USER_HAS_FAVORITE { order: next_order, favorite_timestamp: apoc.date.currentTimestamp() }]->(rb))
RETURN rb, u
`;
