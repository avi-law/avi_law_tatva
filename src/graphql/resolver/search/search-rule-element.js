...
module.exports = async (object, params, ctx) => {
  const { text, showAll, lang } = params;
  ...
  try {
    ...
    if (ruleElementResult && ruleElementResult.records.length > 0) {
      const ruleElements = [];
      ruleElementResult.records.forEach((record) => {
        const resList = record.get("res");
        const res = {};
        resList.forEach((reState, index) => {
        ...
        if (res && Object.keys(res).length > 0) {
          Object.keys(res).forEach((e) => {
            ...
            if (showAll) {
              ...
              resStatus === constants.RULE_ELEMENT_STATE_STATUS.GREEN
            ) {
              ruleElements.push({
                re,
                res: res[e],
